...
async function loadBattery() {
    ...
    try {
        ...
        setText(
            "batteryVoltage",
            finiteNumber(battery.voltage) === null
                ? "N/A"
        ...
        setText(
            "batteryCurrent",
            finiteNumber(battery.current) === null
                ? "N/A"
                : formatNumber(battery.current, 1) + " A"
        );
        ...
        setText(
            "batteryPower",
            finiteNumber(battery.power) === null
                ? "N/A"
                : formatNumber(battery.power, 1) + " W"
        );
...
document.addEventListener(
    ...
    function () {
        ...
        if (window.evDigitalTwinInitialized) return;
...
async function runHistoricalTrendAnalysis(historyResponse = null) {
    ...
    try {
        ...
        const historyValue = (item, names) => {
            for (const name of names) {
                const value = finiteNumber(item[name]);
        ...
        const records =
            history.map(function (item) {
                ...
                return {
                    ...
                    voltage: historyValue(item, ["voltage", "battery_voltage", "battery_voltage_V", "Voltage_Mean_V"]),
                    current: historyValue(item, ["current", "battery_current", "battery_current_A", "Current_Mean_A"]),
                    temperature: historyValue(item, ["temperature", "battery_temperature", "battery_temperature_C", "Temperature_Mean_C"]),
                    batteryPower: historyValue(item, ["power", "battery_power", "battery_power_W"]),
                    loadPower: historyValue(item, ["load_power", "load_power_W"]),
                    soh: historyValue(item, ["SOH_percent", "soh_percent", "Predicted_SOH_percent", "predicted_SOH_percent", "SOH", "soh"]),
                    tyrePressure: historyValue(item, ["tyre_pressure", "tyre_pressure_psi", "tire_pressure"]),
                    speed: historyValue(item, ["speed_kmph", "gps_speed_kmph", "speed"]),
...
function analyzeLoadCapacity(batteryData, vehicleData) {
    ...
    try {
        ...
        const voltage = finiteNumber(batteryData.voltage);
        const current = finiteNumber(batteryData.current);
        const reportedPower = finiteNumber(batteryData.power);
        const batteryPower = reportedPower ?? (voltage !== null && current !== null ? Math.abs(voltage * current) : null);
        ...
        if (
            ...
            loadPower = finiteNumber(vehicleData.load_power);
        ...
        if (loadPower === null && batteryPower !== null) loadPower = batteryPower;
        ...
        if (batteryPower !== null && batteryPower > 0 && loadPower !== null) {
        ...
        if (loadPercentage !== null) loadPercentage = Math.max(0, Math.min(100, loadPercentage));


        console.log(
            "Load percentage:",
            loadPercentage === null ? "Unavailable" : loadPercentage.toFixed(2) + "%"
        );
        ...
        let loadStatus = loadPercentage === null ? "UNAVAILABLE" : "NORMAL";
        let recommendation = loadPercentage === null ? "Load power is unavailable because voltage/current data is incomplete." : "LOAD ACCEPTABLE";


        if (loadPercentage !== null && loadPercentage < 60) {
        ...
        const soh = finiteNumber(batteryData.soh);
        const temperature = finiteNumber(batteryData.temperature);
        ...
        if (loadPowerElement) {
            ...
            loadPowerElement.textContent =
                loadPower === null ? "Unavailable" : loadPower.toFixed(2) + " W";
...
function analyzeRoadCondition(vehicleData) {
    ...
    try {
        ...
        const speed = finiteNumber(vehicleData.speed_kmph ?? vehicleData.speed);
        const tyrePressure = finiteNumber(vehicleData.tyre_pressure ?? vehicleData.tyrePressure);
        ...
        let roadCondition = vehicleData.road_condition || "UNAVAILABLE";
        let roadMessage = speed === null
            ? "Road analysis is unavailable because vehicle speed was not supplied."
            : "Road condition is based on the available vehicle telemetry.";
...
async function runStep43Analysis(cachedBattery = null, cachedVehicle = null) {
    ...
    try {
        ...
        if (cachedBattery && cachedVehicle) {
            const loadResult = analyzeLoadCapacity(cachedBattery, cachedVehicle);
            const roadResult = analyzeRoadCondition(cachedVehicle);
            console.log("STEP 43 COMPLETE", { loadResult, roadResult });
            return;
        }
